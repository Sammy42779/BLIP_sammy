...
class coco_karpathy_caption_eval_adv_save(Dataset):
    def __init__(self, transform, image_root, ann_root, split, max_words=30, prompt=''):  
    # def __init__(self, ann_file, transform, image_root, max_words=30, prompt=''):
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(image_path).convert('RGB')
        image = self.transform(image)        
        img_id = self.image[index].split('/')[-1].strip('.jpg').split('_')[-1]
        
        text = self.prompt + self.text[index]
        image_name = self.image[index]

        return image, int(img_id), text, index, image_name
...
class coco_karpathy_caption_eval_adv_sga_save(Dataset):
    def __init__(self, transform, image_root, ann_root, split, max_words=30, prompt=''): 
    ...
    def __getitem__(self, index):
        ...
        texts = [self.text[i] for i in self.img2txt[index]]  
        img_id = ann['image'].split('/')[-1].strip('.jpg').split('_')[-1]
        # return image, texts, index, text_ids, image_name  # 这里是保存index，而不是image的id
        return image, texts, int(img_id), index, text_ids, image_name

    def collate_fn(self, batch):  # 当我们使用DataLoader来批量获取数据时, collate_fn用于定义如何处理不同的数据样本以将其组合成一个批次
        imgs, txt_groups, img_ids, images_idx, text_ids_groups, image_names = list(zip(*batch))        
        imgs = torch.stack(imgs, 0)
