...
class SGAttacker():
    def __init__(self, net, sga_img_attacker, sga_txt_attacker, *args, **kwargs):
        self.net=net
        self.sga_img_attacker = sga_img_attacker
        self.sga_txt_attacker = sga_txt_attacker
    
    
    def attack_caption(self, images, texts, txt2img, device, max_length=40, scales=None, k=10, **kwargs):
    
        # STEP 1: 根据原始干净图像给每一个文本生成对抗文本
        # txt_attacker.img_guided_attack(self.model, txts, img_embeds=img_supervisions)
        with torch.no_grad():
            normalized_imgs = self.sga_img_attacker.image_normalize(images)[txt2img]
            texts_adv = self.sga_txt_attacker.attack(self.net, normalized_imgs, texts)  
        ...
        # STEP 2: 1) 每个图像resize到5个scale; 生成的对抗样本是没有normalize的; [0,1]
        images_adv = self.sga_img_attacker.txt_guided_attack(self.net, images, texts_adv_prompt_for_loss, txt2img, device, scales=scales)

        # STEP 3: 根据生成的对抗图像给每一个文本生成最终的对抗文本
        with torch.no_grad():
            normalized_adv_imgs = self.sga_img_attacker.image_normalize(images_adv)[txt2img]
            texts_adv = self.sga_txt_attacker.attack(self.net, normalized_adv_imgs, texts)
        
        return images_adv, texts_adv

                

class SGAImageAttacker():
    def __init__(self, image_normalize, eps=2/255, steps=10, step_size=0.5/255):
    ...
    def txt_guided_attack(self, net, imgs, txts, txt2img, device, scales=None):
        ...
        for i in range(self.steps):
            ...
            #     adv_imgs_output = model.inference_image(self.image_normalize(scaled_imgs))
            # else:
            ...
            with torch.enable_grad():
                ...
                for i in range(scales_num):
                    # 根据scale image和对抗文本计算loss
                    loss_item = net.get_lm_loss(self.image_normalize(scaled_imgs)[i*b:i*b+b][txt2img], txts, loss_reduction='mean')
...
class SGATextAttacker():
   # attack text 时候需要image信息
    ...
    def attack(self, net, images, texts, k=10, num_perturbation=1, threshold_pred_score=0.3, max_length=40, batch_size=32):
        ...
        final_adverse = []
        for i, text in enumerate(texts_with_no_prompt):  # 如果不在外层duplicate image, 那么此时是一张图像对应了5个文本, 那么就不能用i一一对应; len(texts_with_no_prompt)=20; len(images)=5  现在在外层把images复制到和text一样的数量
