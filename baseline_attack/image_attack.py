...
class ImageAttacker():
    ...
    def attack(self, image, num_iters):
        ...
        for i in range(num_iters):
            ...
            image_diversity = self.input_diversity(image + self.delta)  # 在这一步之后image因为加了delta而有了grad_fn
            #plt.imshow(image_diversity.cpu().detach().numpy()[0].transpose(1, 2, 0))
            if self.preprocess is not None:  # Normalize
                image_diversity = self.preprocess(image_diversity)  # 用image_diversity进模型,所以是normalize之后的数据
            ...
            self.delta = self.project(self.delta, self.epsilon)  # 算出来delta
            # constraint 2: image range
            self.delta = torch.clamp(image + self.delta, *self.bounding) - image  # 每一次都是修改的delta

        yield (image + self.delta).detach()  # 最后得到的结果是image+delta, 此时是没有normalize的
